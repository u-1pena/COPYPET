  ...
  class Create {
    ...
    void ペット登録されているユーザーを検索した場合例外処理をかえすこと() {
      doReturn(Optional.of(new Pet(1, "testPet", 0, Gender.MALE, 0, 0, 0, 0, 0, 0, 0, 0, 0)))
          .when(petMapper).checkPetAlreadyRegisteredByUserId(1);
      assertThatThrownBy(() -> petService.checkPetAlreadyRegisteredByUserId(1))
    ...
    void ペット登録されていないユーザーを検索すること() {
      doReturn(Optional.empty()).when(petMapper).checkPetAlreadyRegisteredByUserId(1);
      Optional<Pet> actual = petService.checkPetAlreadyRegisteredByUserId(1);
      assertThat(actual).isEmpty();
      verify(petMapper).checkPetAlreadyRegisteredByUserId(1);
    }
